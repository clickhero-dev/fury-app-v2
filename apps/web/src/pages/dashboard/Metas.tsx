  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-sm">
        <span className="font-medium text-[#9A9D96]">{label}</span>
        <div className="flex items-center gap-3">
          <span className={cn('font-bold', isGood ? 'text-[#1E88A8]' : 'text-[#da3633]')}>
            {currentLabel}
          </span>
          <span className="text-xs text-[#8A8D86]">/ meta {targetLabel}</span>
        </div>
      </div>
      <div className="h-2 w-full overflow-hidden rounded-full bg-[#1F211D]">
        <div
          className={cn(
...
export function Metas() {
  ...
  // Garantindo o cálculo exato do mês no frontend
  const today = new Date();
  const dayOfMonth = today.getDate();
  const daysInMonth = getDaysInMonth();
  const daysRemaining = Math.max(0, daysInMonth - dayOfMonth);

  // Verificação no frontend se há metas cadastradas
  ...
  const statusClass = isOnTrack
    ? 'border-[#1E88A8]/30 bg-[#1E88A8]/10 text-[#ECEDEF]'
    : isWarning
      ? 'border-[#CF6F03]/30 bg-[#CF6F03]/10 text-[#ECEDEF]'
      : 'border-[#da3633]/30 bg-[#da3633]/10 text-[#ECEDEF]';
  ...
  return (
    <AppLayout>
      <div className="mx-auto w-full max-w-5xl space-y-8 px-6 py-10 text-[#ECEDEF] sm:px-10 lg:py-14">
        <PageHeader
          title="Minhas Metas"
          ...
          actions={
            <button
              ...
              className="inline-flex items-center justify-center rounded-full border border-[#262824] bg-[#1F211D] px-4 py-2 text-xs font-semibold text-[#ECEDEF] transition-all hover:border-[#1E88A8] hover:text-[#1E88A8]"
            >
              Editar metas
            </button>
          }
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className={`${SURFACE} space-y-5 p-6`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="flex size-8 items-center justify-center rounded-lg bg-[#1F211D]">
                  <Target className="size-4 text-[#1E88A8]" />
                </div>
                <h3 className="font-semibold text-[#ECEDEF]">Metas configuradas</h3>
              </div>
            ...
            <div className="space-y-4 divide-y divide-[#262824]">
          ...
          <div className={`${SURFACE} space-y-5 p-6`}>
            <div className="flex items-center gap-2.5">
              ...
              <h3 className="font-semibold text-[#ECEDEF]">Performance atual vs meta</h3>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-between pt-1 text-sm">
                <span className="font-medium text-[#9A9D96]">ROAS atual</span>
                <span
                  className={cn(
                    'text-base font-bold',
            ...
            <div className={cn('mt-2 flex items-center gap-3 rounded-xl border px-4 py-3', statusClass)}>
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-[#9A9D96]">
        ...
        <div className={`${SURFACE} space-y-5 p-6`}>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-semibold text-[#ECEDEF]">Projeção do mês</h3>
              <p className="mt-0.5 text-xs text-[#9A9D96]">
...
function GoalRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-4">
      <span className="text-sm text-[#9A9D96]">{label}</span>
      <span
        className={cn(
          'text-sm font-semibold text-right',
          highlight ? 'text-[#1E88A8]' : 'text-[#ECEDEF]'
        )}
      >
        {value}
      </span>
...
function ProjectionCard({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div className="space-y-1.5 rounded-xl border border-[#262824] bg-[#1F211D]/50 p-4">
      <p className="text-xs font-medium uppercase tracking-wider text-[#9A9D96]">{label}</p>
      <p className="text-2xl font-black text-[#ECEDEF]">{value}</p>
      <p className="text-xs text-[#8A8D86]">{sub}</p>
    </div>
  );
}