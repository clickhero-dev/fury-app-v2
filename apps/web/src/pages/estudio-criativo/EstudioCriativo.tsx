...
export function EstudioCriativo() {
  ...
  const creativeColumns = [
    ...
    },
    {
      ...
      label: 'Tipo',
      render: (value: any) => (
        <span className="inline-flex items-center gap-2 px-3 py-1 bg-accent-light/10 text-accent rounded-lg text-xs font-semibold">
    ...
    },
    {
      ...
      render: (value: any) => (
        <span className={value.includes('↓') ? 'text-error font-semibold' : 'text-success font-semibold'}>
          {value}
        </span>
      ),
    },
              ...
              {tabs.map((tab) => (
                <button
                  ...
                  className={`flex-1 px-6 py-4 font-semibold text-sm transition-colors border-b-2 ${
                    selectedTab === tab.id
                      ? 'border-[#E8631A] text-accent'
                      : 'border-transparent text-text-secondary hover:text-text-primary'
                  }`}
                >
                  {tab.label}
                  <span className="ml-2 inline-flex items-center justify-center w-5 h-5 rounded-full bg-surface-secondary text-xs font-bold text-text-secondary">
                    {tab.count}
                  </span>
                </button>
              ))}
        ...
        <Card>
          <div className="p-6 space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-bold text-text-primary">Dica: Otimize suas Criações</h4>
