...
import type { InstagramService, InstagramRankingObjective } from '../services/meta/instagram.service.js';
...
/** Controller Instagram — glue HTTP fino. Recebe o service no construtor (injeção). */
export class InstagramController {
  ...
  getPostsRanked = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const parsed = postsRankedQuerySchema.parse(req.query);
      const objective = parsed.objective as InstagramRankingObjective;
      const posts = await this.instagramService.getRankedPosts(tenantId, objective, parsed.instagramUserId);
  ...
  mediaProxy = async (req: Request, res: Response, next: NextFunction) => {
    try {
