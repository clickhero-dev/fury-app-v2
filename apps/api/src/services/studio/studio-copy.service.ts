...
async function runTextComplianceCheck(text: string) {
  ...
  try {
    const response = await openrouterService.chat(
      ...
      { model: 'deepseek/deepseek-chat', max_tokens: 512, response_format: { type: 'json_object' } },
    );
...
export async function generateAdCopy(input: AdCopyInput, tenantId: string) {
  ...
  const text = await openrouterService.chat(
    ...
    { model: 'deepseek/deepseek-chat', max_tokens: 1200, response_format: { type: 'json_object' } },
  );
